...
/*
1. Gsearch seems to be the biggest driver of business. Pulling monthly trends for Gsearch sessions 
and orders so that we can showcase growth here. 
*/
...
/*
2. Next, it would be great to see a similar trend for Gsearch, but this time splitting out nonbrand and brand
campaign separately.
*/
...
/*
3. While we're on Gsearch, we will dive into nonbrand, and pull monthly sessions and orders split by 
device type. 
*/
...
/* concerned about the large % of traffic is from Gsearch. Pulling monthly trends for Gsearch, 
alongside monthly trend for each of our other channel.
*/
...
/*
5. Checking the story of our website performance improvement over the course of the first 8 months.
...
-- ANALYZING SEASONALITY AND BUSINESS PARTNERS
-- Analyzing business patterns is about generating insights to maximize efficiency and anticipate future trend.alter
-- USING SQL date functions here.

-- We should take a look at 2012 monthly and weekly volume patters to see if we can find any seasonal trend 
-- we should plan for in 2024
-- we can pull session volume and order volume. 
...
-- Looks like we grew fairly steadily all year and saw significant volume around the holiday month 
-- (especially the week of black friday and cyber monday)
-- We want to keep in mind in 2013 as we think about customer support and inventory management.

-- Now we will analyze the average website session volume by hour of day and by day week so that we can staff appropriately.
-- Lets avoid the holiday time period and use a date range of sep 15 - nov 12 2012.
...
-- Looks like 10 sessions per hour per employee staffed is about right 
-- Looks like then we can plan on one support staff around the clock and then we should double up to 
-- two staff member from 8 am to 5 pm MOnday through friday.